...
- [Numeric D2 refined widening landed — implicit same-kind widen at hint sites](decisions/types.md#2026-05-10-numeric-d2-refined-widening-landed) — Same-kind narrower source flows into a wider hint without an explicit cast (`Int8 → Int`, `Int32 → Int64`, `UInt8 → UInt32`); the lowerer inserts the Go conversion (`int(int8val)`) at the lowerExprHint boundary so emit produces valid Go. Asymmetric (narrowing still requires `T(x)?`); cross-kind still routes through the explicit cast or cross-base diagnostic. Closes the B4 motivating case (`let id: Int = res.LastInsertId()?` for `int64 → int`) without forcing user-side ceremony. Originally deferred from Slice E for scope but the user flagged it as essential — the design memo's "value-flow widening" promise was making the original B4 motivation flow naturally.
- [Numeric Slice I landed — `stdlib.BigInt` arbitrary-precision arithmetic](decisions/types.md#2026-05-10-numeric-slice-i--stdlibbigint-landed) — `BigInt` wraps Go's `math/big.Int` with an immutable Arca surface (operations always allocate fresh results). Covers `NewBigInt(v: Int)`, `BigIntFromString`, `Add / Sub / Mul / Div / Mod / Neg`, `Eq / Lt / Gt`, `String`, `ToInt`. The third numeric layer alongside fast+panic Int and Result-returning std.checked. Heap-allocated and slower than Int; opt-in via explicit factory call. Slice G (B4 verification) now the only remaining numeric task and is on the user side.
- [Numeric Slice E landed — literal coercion, cross-base diag, arithmetic panic, std.checked](decisions/types.md#2026-05-10-numeric-slice-e--literal-coercion-cross-base-arithmetic-panic-stdchecked-landed) — Hint-driven literal coercion (`let x: Int8 = 100`) types the literal at the hint's Go type and statically rejects out-of-range values. Cross-base operand mismatch (Int + UInt) raises a dedicated compile error directing the user to `T(x)?`. `+ - *` on Int / UInt operands lower to panic-checked helpers (`__addInt`, `__subUInt`, `__mulInt`, ...) so silent overflow is no longer a Layer 1 leak; narrow operands widen to base before the helper. `stdlib.CheckedAddInt` … `CheckedDivUInt` provide Result-returning checked variants (8 functions) for callers that need recoverable errors. Numeric trait (Bindable-style intrinsic) remains the proposed surface for `a.checkedAdd(b)?` ergonomics — deferred to a future Slice J pending real demand.
- [Numeric Slice D+F+H landed — tower types, `T(x)?` cast, Bindable narrow](decisions/types.md#2026-05-10-numeric-slice-dfh--tower-cast-bindable-narrow-landed) — `Int8` … `Float64` available as built-in primitives mapping to the corresponding Go type. `T(x)?` cast for tower targets and base `Int` / `UInt` / `Float` lowers to a synthesised `New<T>(v int64 | uint64 | float64) (<go>, error)` validator gated by per-type builtin keys. Bindable Draft slots widen narrow numeric fields to int64 / uint64 / float64 so SQL Scan + JSON UnmarshalJSON deliver values loss-lessly; Freeze re-narrows via the same `New<T>` validators (single validation path for both user-explicit casts and Bindable population). Original design's α (`BindableDict.ScanRaw` per-field validator slice) replaced with the simpler δ approach since user-defined narrow constraints already follow this Freeze-time pattern via `NewT`.
- [Numeric Slice C landed — `bits: N` storage hint](decisions/types.md#2026-05-10-numeric-slice-c--bits-n-storage-hint-landed) — `Int{bits: N}` / `UInt{bits: N}` / `Float{bits: N}` selects the Go storage type (`int8` / `uint32` / `float32` / etc.) without a runtime check. Works at type aliases (`type Int32 = Int{bits: 32}`) and inline at struct fields. Validators skip `bits` (it's a storage hint, not a check); invalid widths and non-numeric bases raise `ErrInvalidBitsConstraint`. Unblocks the stdlib numeric tower (Slice D+F+H bundle) where `Int8` … `Float32` are defined as constrained aliases.
- [Numeric Slice B landed — `UInt` core type](decisions/types.md#2026-05-10-numeric-slice-b--uint-core-type-landed) — `UInt` resolves to Go `uint` end-to-end through `lowerNamedType`, `arcaDisplayName`, `irTypeToGoString`, `irZeroExpr`, `typeZeroValue`, and OpenAPI schema (with `minimum: 0`). Symmetric with `Int`. Closes the UInt64 representation gap surfaced by B4 (MySQL `BIGINT UNSIGNED`, hash output, file mode). Literal hint coercion (`let x: UInt = 5`) deferred to Slice E.
- [Numeric Slice A landed — 64-bit GOARCH enforcement](decisions/types.md#2026-05-10-numeric-slice-a--64-bit-goarch-enforcement-landed) — Emit injects `//go:build amd64 || arm64 || ppc64 || ppc64le || mips64 || mips64le || riscv64 || s390x || loong64 || wasm` at the top of every generated Go file; `arca run` / `arca build` precheck `GOARCH` (env override → `runtime.GOARCH` fallback) and refuse to invoke the Go toolchain on a 32-bit target. Grounds `Int = Go int` at 64 bits before any tower / arithmetic work.
- [Numeric types — core + constrained type tower (design)](decisions/types.md#2026-05-10-numeric-types--core--constrained-type-tower) — Three core types `Int = Go int` (64-bit-only enforced), `UInt = Go uint` (added to handle UInt64 representation gap), `Float = float64`. stdlib/numeric tower (`Int8/16/32/64`, `UInt8/16/32/64`, `Float32`) expressed as constrained types via `{bits: N}` constraint key. D2 refined extends to value flow and arithmetic (range-aware widening). Existing `T(x)?` constructor syntax extends to numeric narrowing — struct ctor, constrained ctor, and numeric narrowing share one syntax. Arithmetic overflow on base types emits panic (Layer 1 violation detection signal). `std.checked.*` prelude provides opt-in Result-returning safe arithmetic; `std.bigint` provides arbitrary-precision arithmetic as the third numeric layer. Bytes-as-dedicated-type rejected; `List[Byte]` (= Go `[]byte`) sufficient. Slices A–G+I detailed in design memo `design_numeric_types.md`.
- [Layer 1 expansion + panic policy revision](decisions/foundations.md#2026-05-10-layer-1-expansion--panic-policy-revision) — Layer 1 motivation reframed as "seal every path that can deliver invalid values to SSOT data", with seal mechanism chosen by path nature (internal bug → panic emit / business error → Result / FFI-derived → boundary wrap). Old "panic 全面禁止" policy revised to "panic is the Layer 1 violation detection signal", permissible for arithmetic overflow / slice OOB / div-zero / structural unreachable / explicit assert (request-scoped recovery via net/http assumed). Aligns with modern trend (Rust 0560 RFC, Swift trap-default, Zig illegal-behavior). Arithmetic overflow added to Layer 1 list. Deferred candidates: string UTF-8 invalid, Float NaN/Inf, refinement-lite range tracking through `let` bindings.

## 2026-04-23

- [Callable IR unification implemented (slices U1–U6)](decisions/ideas.md#2026-04-22-callable-ir-unification--one-ir-for-method--lambda--function-design) — `IRFuncDecl`, `IRLambda`, and `IRFnCall.Func string` collapse into one `IRFn{GoName, Receiver, Params, Ret, Body, Source}`; `IRFnCall.Fn IRExpr` replaces the string Func. Inline lambdas as callees flow through Fn instead of the former `/* complex call */` fallthrough. U6 wired the monadic lambda hint channel via `applyLambdaHint` (extracted from `lowerLambdaHint`) so `.map(u => u.)` completion resolves `u`'s fields end-to-end. Supporting fixes: non-Block lambda body scope range via `exprFirstPos`, synthetic match-arm `EndPos` fallback, lambda-param `ArcaType` propagation, cursor-aware placeholder insertion.
- [Function types F5–F8 landed](decisions/ideas.md#2026-04-22-function-types-as-first-class--higher-order-functions-across-the-language-design) — F5: `BuiltinDef.Signature(l) PreludeSig` produces fresh-typevar `IRFnType` per call so `map` / `filter` / `takeWhile` / `take` / `fold` flow through HM; `lowerPreludeArgs` + `inferPreludeReturnType` retired. F6: `goTypeToIR` recognises `func(...)` → `IRFnType` and `resolveCallParamIRType` unifies Arca-fn / Go-FFI / Go-method hint computation; `parseFuncType`, `inferLambdaParamTypes`, `goTypeToArcaType`, `resolveCallParamFuncType` retired. F7: hover and go-to-definition already render fn types as `A -> B` via `irTypeDisplayStr`; regression-locked with a test. F8: SPEC.md / DESIGN.md / CLAUDE.md synced with the function-types feature.
- [Parser error recovery for LSP (idea)](decisions/ideas.md#2026-04-23-parser-error-recovery-for-lsp-idea) — `insertCompletionPlaceholder`'s dot-patch approach compounds as partial-input shapes accumulate (unclosed parens, half-typed match arms, partial type annotations). TypeScript / rust-analyzer solve this in the parser via sync points + error nodes, so LSP / build / tests benefit uniformly. Deferred; trigger is the next partial-input pain point.
- [Reject unknown bare type names (idea)](decisions/ideas.md#2026-04-23-reject-unknown-bare-type-names-idea) — `lowerNamedType`'s default branch silently accepts typos like `int` (Go primitive) or `Usre`. Tighten to reject unresolved bare names with a "did you mean Int / String / Bool / ..." hint for Go primitives. Dotted FFI types stay opaque.
- `arca version` now reports the commit hash and dirty flag (`v0.1.0 (abc1234-dirty)`) via `debug.ReadBuildInfo`, so a built binary's provenance is visible at a glance.

## 2026-04-22

- [Callable IR unification (design)](decisions/ideas.md#2026-04-22-callable-ir-unification--one-ir-for-method--lambda--function-design) — F4 work on function types exposed that `IRFuncDecl` / `IRLambda` / `IRFnCall.Func string` are three representations of the same concept, and the string `Func` field cannot carry a function-valued expression. Inline lambda calls fall through the "complex call" fallthrough in `lowerFnCall` and fail LSP scope registration + emit at once. Unify into one `IRFn` struct with `IRFnCall.Fn IRExpr`; decl and literal share one renderer. Slices U1–U6, prerequisite for F4–F8.
- [Function types F1–F3 landed (implementation)](decisions/ideas.md#2026-04-22-function-types-as-first-class--higher-order-functions-across-the-language-design) — F1 parser + AST (`FunctionType`), F2 IR + lower + emit (`IRFnType`, structural invariant unify), F3 `lowerLambdaHint` consumes IRFnType so any hint-receiving site (let annotation, fn return, arg position) types the lambda params without per-site code. F4 (monadic desugar) attempted and reverted — blocked on the callable unification slice above.
- [Function types as first-class (design)](decisions/ideas.md#2026-04-22-function-types-as-first-class--higher-order-functions-across-the-language-design) — Add AST `FunctionType` and IR `IRFnType` to make higher-order functions a genuine language feature. `examples/todo` migration surfaced that lambda-param inference lives in three parallel ad-hoc paths (Go FFI `FuncInfo`, prelude `lowerPreludeArgs`, monadic desugar with none) and that `lowerLambdaHint` is a hint-ignoring stub. Unifying these requires reifying function types in the type system: n-ary shape `(A, B) -> C`, right-associative, structural invariant unification, no `==` / `nil` on function values, IRFnType ↔ Go `func(A) B` direct interop. Unlocks `fun apply(f: Int -> Int)` / `let f: A -> B = ...` / function-typed struct fields. Implementation in 8 slices F1–F8 (AST/parser → IR/lower/emit → lowerLambdaHint → migrate monadic/prelude/Go-FFI → LSP → docs).
- [Trait system Phase 1 implemented (slices 4a–4d2)](decisions/ideas.md#2026-04-21-trait-system-phase-1--minimum-viable-traits-for-error-design) — Landed the 2026-04-21 trait design end-to-end. Parser accepts `trait Name { sig }` and `impl Type: Trait { body }`; lower adds `IRTraitType`, trait/impl registries, impl-method-as-Go-method emission, method resolution that walks impls + trait sets, and `traitImplCompatible` unify fallback; emit renders user traits as `type Arca<Name> interface { ... }`. Prelude-registered `trait Error { fun message() -> String }` is available without import. Impls of `Error` get an auto-generated `Error() string` shim so they satisfy Go's stdlib `error` too. Lowercase `error` removed from the user surface: Go FFI `(T, error)` returns map to `Result[T, Error]` at the IR level, and match `Err` bindings wrap the raw Go error in an internal `__goError` adapter (`{Message, Error, Unwrap}`) so trait methods resolve on the bound value. One departure from 2026-04-21: `Error` emits as Go's stdlib `error` rather than a distinct `ArcaError` interface; the distinct-interface goal is met through `__goError` wrapping, which avoids pervasive signature churn. Remaining Phase 1 follow-ups: prelude `StringError` convenience type (4e), `examples/todo` migration (4f).

## 2026-04-21

- [Trait system Phase 1 (design)](decisions/ideas.md#2026-04-21-trait-system-phase-1--minimum-viable-traits-for-error-design) — Minimum viable trait system to unblock the Error trait and `examples/todo`. 18 decisions: separate `impl Type: Trait { ... }` blocks (not inline); orphan rule adopted, Phase 1 further restricted to local-only; trait names are valid type expressions (trait objects); Go emit maps trait to `interface`, impl to methods; dispatch always dynamic; concrete-to-trait coercion automatic at hint sites; method resolution order inherent → impl → trait set, any collision is a compile error; match type pattern extends to trait types without exhaustiveness; implicit `self`, value receivers; `Self` and `static fun` forbidden in trait/impl in Phase 1; inherent `impl` blocks forbidden; Error trait emits as distinct `ArcaError` interface with compiler-generated `Error() string` shim per impl; Go FFI `error` wraps into `__goError` at the boundary. Default methods, trait inheritance, and generic bounds deferred.
