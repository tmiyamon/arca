...
**Decision: `try { ... }` block expression creates a local Result context where `?` is valid.**

```arca
fun main() {
  let result = try {
    let row = db.query("SELECT ...", 1)?
    let user = scan(row)?
    user
  }
...
- `try` is not a keyword — only `try {` is recognized as a try block. `let try = 42` remains valid.
- The final expression in the block is auto-wrapped in `Ok`.
- Emitted as a Go IIFE: `func() (T, error) { ... }()`.
- HM inference: fresh type var allocated for the Ok type, unified with the final expression type. The block's type is `Result[T, Error]`.

**Status:** Done

---

## 2026-04-18: `?` compile error outside Result context

**Context:** `?` outside a Result-returning function previously generated `panic(err)` in Go output. This violated Arca's design principle that generated code should not panic.

**Decision:** `?` outside Result functions and `try {}` blocks is now a compile error. The error message suggests using `try {}` or changing the function return type to `Result`.

**Status:** Done

---

## 2026-04-18: w.Unreachable()

**Context:** Exhaustive match in Go emits a default branch to satisfy the Go compiler (which can't verify Arca's exhaustiveness). Previously this used the same `panic("unreachable")` as intentional runtime assertions, making it impossible to distinguish "structurally impossible" from "programmer assertion".

**Decision:** Separate `w.Unreachable()` GoWriter method for exhaustive match stubs. Semantically distinct from `w.Panic()` even if both emit `panic(...)` in Go. The distinction enables future tooling (dead code analysis, coverage) to treat them differently.

**Status:** Done

---

## 2026-04-18: IR naming: ErrorReturnValues + NilCheckReturnValues

**Context:** `PropagateValues` was the IR field name for the error return path generated by `?` on Result types. Adding Option `?` support (nil check → error return) needed a parallel mechanism.

**Decision:** Rename `PropagateValues` → `ErrorReturnValues` (clearer intent). Add `NilCheckReturnValues` for the Option `?` path that checks for nil and returns an error. Both are fields on `IRTryExpr` / `IRTryLetStmt`.
