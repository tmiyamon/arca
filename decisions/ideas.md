...
**Status:** F1–F8 landed. F1 (AST/parser), F2 (IRFnType + lower/unify/emit), F3 (hint-driven `lowerLambdaHint`) landed 2026-04-22. F4 redone on the unified callable-IR base as slice U6 (see 2026-04-22 "Callable IR unification" entry). F6 landed 2026-04-23: `goTypeToIR` recognises `func(...)` and returns `IRFnType`; `resolveCallParamIRType` unifies the Arca-fn / Go-FFI / Go-method hint paths. F5 landed 2026-04-23: prelude `BuiltinDef` gains a `Signature(l) PreludeSig` that produces a fresh-typevar IRFnType per call; `map` / `filter` / `takeWhile` / `take` / `fold` route through `lowerExprHint` and HM unification, with `lowerPreludeArgs` and `inferPreludeReturnType` retired. F7 verified 2026-04-23: `irTypeDisplayStr` and `typeName` already render fn types as `A -> B`, hover and go-to-definition work on fn-typed bindings; completion Detail for fn values is not applicable under Arca's `.`-trigger-only completion model. F8 synced docs 2026-04-23: SPEC.md Function Types subsection + built-in types row, DESIGN.md Function Types section (n-ary over curried, no function equality, single hint channel), CLAUDE.md Key Design Decisions bullet.
...
**Status:** U1–U6 landed 2026-04-23. `IRFn{GoName, Receiver, Params, Ret, Body, Source}` is the single callable representation; `IRFuncDecl` is a type alias; `IRLambda` is removed; `IRFnCall.Fn IRExpr` replaces the former `Func string`. `/* complex call */` fallthrough is deleted — inline lambdas as callees now flow through `Fn`. Emit renders decls via `emitFn` and literals via `emitLambda`; both take `IRFn`. U6 wired the monadic `LamArg` signature into `applyLambdaHint` (shared with `lowerLambdaHint`) via `tryDesugarMonadicMethod`, so `.map(u => u.)` completion resolves the lambda param's fields.
