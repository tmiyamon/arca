...
| `target_arch.go` | 64-bit GOARCH allowlist + `//go:build` tag (SSOT for emit + CLI precheck) |
...
- **`?` in expression position**: `?` is a postfix operator parsed in `parseUnaryExpr` so it binds tighter than binary operators (`f()? * 2` = `(f()?) * 2`). After consuming `?`, `parsePostfixChain` resumes so `.field` / `.method(args)` / `[idx]` chains naturally — `parseUnaryExpr` and `parsePrimaryExpr` (TkIdent branch) share that one helper as SSOT. In any value-position the lowerer produces `IRTryExpr{Inner, OkType, ErrType, ReturnType}`; `stage2Walker` deep-walks data expressions via `hoistTryInExpr`, pushing `__try<N>, __try<N>_err := …` + `if __try<N>_err != nil { return zero, __try<N>_err }` onto a hoist buffer and substituting the expression with `__try<N>`. Statement-producing methods flush the buffer before each emitted stmt. Lambda / try-block boundaries are opaque (each owns its own walker / return type). Statement-level `let x = expr?` and bare `expr?` keep the existing `IRTryLetStmt` shortcut path.
...
- **Bindable (compiler intrinsic)**: `derive Bindable` on a type activates compiler-synthesised dictionary dispatch — never written as a user trait, manual `impl T: Bindable` rejected. `validateDeriveList` records the host in `l.bindableTypes`; `synthesizeBindableTypes` (pre-stage2) emits `BindableSlot[T any]` + `<T>Draft` IRStructDecls; `synthesizeBindableDispatch` (post-stage2) emits a `(d <T>Draft) Freeze` method, a `<typeName>Draft()` factory, and a `__<T>Bindable BindableDict[T, <T>Draft]` global var wiring both via fn-name + Go method expression. Generic functions take `[T: Bindable]`; `lowerFnCommon` expands the type-param list to `[T any, __draftT any]` plus a leading `__bindableT BindableDict[T, __draftT]` value param. Call sites with explicit type args inject `__<Type>Bindable` (or forward the caller's hidden param via `currentFnBindableParams` for transitive chains) and append `<Type>Draft` to the Go type-args. `Bindable` is reserved (intrinsic registry); `BindableDict` has a synthetic AST TypeDecl in `l.types` so `__bindableT.draft/freeze` field access flows through the normal lower path. The runtime types `BindableSlot[T]` (with `UnmarshalJSON` for json populate) and `BindableDict[T, B]` are hosted in `stdlib/bindable.go` — `ensureStdlibImported` auto-imports it whenever a Bindable host exists. Stdlib helpers (`Decode`, `BindJSON`) take the dict as a hidden first param; `maybeInjectBindableStdlibArg` rewrites the call site to inject `__<Type>Bindable` plus the `<Type>Draft` Go type arg. The two-Go-type-param leak (`T`, `__draftT`) is unavoidable — Go has no associated types to express `T.Draft` as a single bound. See `decisions/ffi.md` 2026-05-04 (refined).
