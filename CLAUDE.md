...
- **GoMultiReturn**: Go FFI calls carry `GoMultiReturn` flag + `IRResultType`/`IROptionType`. `goFuncReturnType` maps `(T, error)` → Result, `(*T, error)` → `Result[Option[*T], Error]`, `(T, bool)` → Option, `*T` → `Option[*T]`, 3+ → Tuple. Consumption sites read IR types, no ad-hoc detection.
- **Pointer Option auto-wrap**: Go `*T` returns wrapped in `IROptionType` at the IR level. `?` double-unwraps with nil check via `NilCheckReturnValues`.
